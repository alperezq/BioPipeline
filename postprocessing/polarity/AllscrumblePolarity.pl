...
if ($argc != 4)   
{
    print "usage: \n perl scrummbleTest.pl sequence_file cost_file outputfilename_for_random_data arlem_in_full_path\n";
    exit;
}
my $seq_file = $ARGV[0];
my $costfile = $ARGV[1];
...
$argstring="scrummbleTestPolar.pl $seq_file $costfile 0 $seq_file.polarscrumb $mod_structu_flag $breakeven_flag $arlem";
...
for($i=0;$i<$numberofsimulations;$i++){    
    $argstring="scrummbleTestPolar.pl $seq_file $costfile $numberofiterations $outputfile $mod_structu_flag $breakeven_flag $arlem";    
    $retcode=system($argstring);
