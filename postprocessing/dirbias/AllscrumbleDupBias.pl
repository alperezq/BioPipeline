...
if ($argc != 5)   
{
    print "usage: \n perl AllscrumbleDupBias.pl sequence_file cost_file number_of_simulations outputfilename arlem_in_full_oath\n";
    exit;
}
my $seq_file = $ARGV[0];
my $costfile = $ARGV[1];
...
$argstring="scrummbleTest.pl $seq_file $costfile 0 $seq_file.evaluated $mod_structu_flag $breakeven_flag $arlem";
$retval=system($argstring);
if($retval!=0){
    print "Error running: $argstring\n";
    exit -1;
...
for($i=0;$i<$numberofsimulations;$i++){    
    $argstring="scrummbleTest.pl $seq_file $costfile $numberofiterations $outputfile $mod_structu_flag $breakeven_flag $arlem";
    $retval=system($argstring);
    if($retval!=0){
	print "Error running: $argstring\n";
	exit -1;
