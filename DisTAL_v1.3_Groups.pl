...
my $height;
if ($ARGV[2]) {$height = "$ARGV[2]"} else {$height = 4.5};
...
if (defined $ARGV[3] && ($database eq "F")) {
my $count = 0;
open my $ADD, "<$ARGV[3]" or die "Can not open $ARGV[3], file doesn't exist\n";
while ( my $line = <$ADD> ) {
	chomp $line;
...
tr <- nj((exprs))
a <-(hclust(as.dist(exprs))) #Cut tree and get groups
