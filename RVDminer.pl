...
for (my $j = 0; $j <= $#AoA; $j++) { #FOR EACH TAL in the plus strand
	...
	my $fname;
	$fname = (split(/\./,basename("$ARGV[0]")))[0];
	my $nuheader = "$fname|$header"."_+_$ATG"."_$STOP"."_$tot";
...
for (my $j = 0; $j <= $#AoB; $j++) { #FOR EACH TAL in the MINUS strand
	...
	my $fname = (split(/\./,basename("$ARGV[0]")))[0];
	my $nuheader = "$fname|$header"."_-_$minSTOP"."_$minATG"."_$tot";
	#my $nuheader = "$header";
